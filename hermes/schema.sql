...
--
-- Canonical schema for the Hermes memory plugin (hermes/openltm_hermes).
-- Source: hermes-brain/src/schema.sql (32-object terminal shape), corrected per
-- Max independent review (audit/14, 2026-08-08): migrations 007/011/013 columns
-- (memories.workspace_id/agent_id/decay_score/created_by, context_items.workspace_id/
-- agent_id) are folded into the CREATE TABLE statements so a fresh install from this
-- file alone is column-terminal and the plugin's learn()/janitor paths do not crash.
...
CREATE TABLE IF NOT EXISTS context_items (
  ...
  -- Multi-agent isolation (migration 007). NULL = global / default agent.
  workspace_id TEXT,
  agent_id     TEXT,
...
CREATE TABLE IF NOT EXISTS memories (
  ...
  workspace_id         TEXT,
  agent_id             TEXT,
  -- Pre-computed decay score (migration 011). Janitor materializes it.
  decay_score          REAL    NOT NULL DEFAULT 1.0,
  -- Origin actor (migration 013). NULL until backfilled.
  created_by           TEXT
);
