...
* **Version:** 1.4 (against plugin v2.12.0)
* **Owner:** Rohi Rikman
...
C4Context
  ...
  System_Boundary(host, "Developer Machine") {
    ...
    SystemDb(sqlite, "openltm.db", "Local SQLite (WAL mode) under CLAUDE_PLUGIN_DATA")
...
- `registry.json` is a small JSON map. Concurrent writes are rare (only during
  `/openltm:project register` or `SessionStart` first-touch) but unsynchronised — a
...
sequenceDiagram
  ...
  participant DB as openltm.db (SQLite)
  ...
  Dev->>Claude: makes a decision; runs /openltm:memory learn
...
|------|-----------|-----------|--------------|
| SessionStart | DB, registry | registry (auto-register only) | Hook silent-fail → no injection; `/openltm:doctor` detects |
...
The persistent store is one SQLite file (`openltm.db`) under `${CLAUDE_PLUGIN_DATA}`.
...
- **Reporting:** `/openltm:health` summarises memories by decay bucket
...
`${CLAUDE_PLUGIN_DATA}/openltm.db`. FTS5 virtual table for full-text search.
...
- (+) One-file backup story; `cp openltm.db openltm.db.bak` is a complete export.
...
- (–) Hook failures are silent by default — drove the need for `/openltm:doctor` (US-9).
...
- (–) "Why didn't my fix take effect?" is a known foot-gun. Driver of the
  `/openltm:doctor` checks for cache version vs. installed version.
...
PRD US-10 (`/openltm:admin migrate`) sits on top of this fragile foundation.
...
**Root cause.** No metrics surface. No event log table. `/openltm:health` reads
counts but not timings.

**Direction.** Add an `events` table (append-only): event_type, latency_ms,
result_count, timestamp. Aggregate in `/openltm:health`.
...
flowchart TB
  subgraph Local["Developer Machine (canonical store)"]
    DB[(openltm.db v2: WAL, partitioned, audited)]
...
**Opt-in.** Default off. Configurable via `/openltm:admin sync setup`.
...
were a month ago: `/openltm:project replay --date 2026-03-15`.

### 8.12 C10 — Memory diffing across versions

Pair to C9. `/openltm:memory diff --from 2026-01-01 --to 2026-04-01` returns
...
C4Container
  ...
  System_Boundary(ltm2, "LTM Plugin v2") {
    ...
    ContainerDb(db, "openltm.db v2", "SQLite WAL + partitioned + audited", "memories, embeddings (split), provenance, audit, settings, schema_migrations")
  }
...
- **W1, W5:** Add structured hook + MCP event log table. `/openltm:doctor` reads it.
- **W3:** Replace ad-hoc `runMigrations` with a `schema_migrations` table and
  numbered SQL files. Backfill from current state.
- **W6:** Add a documented "dev mode" symlink helper to `/openltm:admin`.
...
- Add latency / recall-hit telemetry to the DAO; surface in `/openltm:health`.
...
| 6 | Adds tables (sync state) | Adds optional sync tools | New `/openltm:admin sync` group |
| 7 | No | Adds optional tools | New `/openltm:memory diff/replay` |
